...
platform: "ensemble"
max_batch_size: 0
# RoMaV2 "{setting}" dense matcher followed by the shared {sampler} (RoMaV2.sample()).
# Same pipeline for every setting. Written by the export together with the model:
#   python scripts/export_onnx.py --setting {setting}
...
def render(setting: str, size: int | None = None, name: str = DEFAULT_NAME,
    ...
    return {
        ...
        repo / sampled / "config.pbtxt": SAMPLED.format(
            name=dense, sampled=sampled, sampler=SAMPLER, setting=setting, size=size),
    }
