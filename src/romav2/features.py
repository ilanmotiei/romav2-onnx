...
def wrap_with_normalize(
    ...
    *,
    ...
    def wrapped_forward(self, img: torch.Tensor) -> list[torch.Tensor]:
        with torch.autocast(device.type, torch.bfloat16, enabled=enable_amp):
...
class VGG(nn.Module):
    ...
    def forward(self, x):
        x = imagenet(x)
        with torch.autocast(device_type=device.type, enabled=self.enable_amp, dtype=torch.bfloat16):
            feats = {}
            scale = 1
...
class VGG19(VGG):
    def __init__(self, patch_size: int, enable_amp: bool = True) -> None:
        super().__init__()
...
class VGG19BN(VGG):
    def __init__(self, patch_size: int, pretrained: bool, enable_amp: bool = True) -> None:
        super().__init__()
        self.enable_amp = enable_amp
...
class FineFeatures(nn.Module):
    ...
    def __new__(cls, cfg: Cfg):
        match cfg.type:
            case "vgg19":
                return VGG19(cfg.patch_size, enable_amp=cfg.enable_amp)
            case "vgg19bn":
                return VGG19BN(cfg.patch_size, cfg.pretrained, enable_amp=cfg.enable_amp)
            case _:
